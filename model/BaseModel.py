...
class WordEmbedding():
    def __init__(self, num_steps=128, dict_size=20000, word_vec_size=100):
        with tf.name_scope('wordembedding'):
            ...
            #self.saver = tf.train.Saver([var for var in tf.trainable_variables() if 'wordembedding' in var.name])

class BiRnn():
    def __init__(self, inputs, rnn_type = 'lstm', rnn_size_list=[10,10], is_train=False, rnn_keep_prob=0.99):
        with tf.name_scope('birnn'):
            self.sequence_length = tf.placeholder(tf.int32, [None], name='sequence_length')
            if rnn_type=='lstm':
                cell_fw = tf.nn.rnn_cell.MultiRNNCell([rnn_cell.LSTMCell(rz, state_is_tuple=True) for rz in rnn_size_list])
                cell_bw = tf.nn.rnn_cell.MultiRNNCell([rnn_cell.LSTMCell(rz, state_is_tuple=True) for rz in rnn_size_list])
            elif rnn_type=='gru':
                cell_fw = tf.nn.rnn_cell.MultiRNNCell([rnn_cell.GRUCell(rz) for rz in rnn_size_list])
                cell_bw = tf.nn.rnn_cell.MultiRNNCell([rnn_cell.GRUCell(rz) for rz in rnn_size_list])
            ...
            #self.saver = tf.train.Saver([var for var in tf.trainable_variables() if 'birnn' in var.name])

class CRF():
    def __init__(self, inputs, num_tags=2, sequence_length=None):
        with tf.name_scope('crf'):
            ...
            #self.saver = tf.train.Saver([var for var in tf.trainable_variables() if 'crf' in var.name])

class Classiffier():
    def __init__(self, inputs, num_label, ffn_units_list=[]):
        with tf.name_scope('classiffier'):
            ...
            #self.saver = tf.train.Saver([var for var in tf.trainable_variables() if 'classiffier' in var.name])
            

class Regression():
    def __init__(self, inputs, num_out, ffn_units_list=[]):
        with tf.name_scope('regression'):
            ...
            #self.saver = tf.train.Saver([var for var in tf.trainable_variables() if 'regression' in var.name])
...
class TestModel():
    def __init__(self):
        ...
        self.brnn = BiRnn(inputs=self.we.outputs)
